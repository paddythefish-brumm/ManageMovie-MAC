...
on run
  ...
  repeat
    set initialSetupDone to do shell script "/bin/bash -lc " & quoted form of "curl -kfsS --max-time 2 https://127.0.0.1:8126/api/state 2>/dev/null | /usr/bin/python3 -c 'import sys, json\ntry:\n d=json.load(sys.stdin)\n print(1 if d.get(\"settings\", {}).get(\"initial_setup_done\") else 0)\nexcept Exception:\n print(0)'"
    ...
    if initialSetupDone is not "1" then
      set beginning of actionItems to "Lade Post-MaMo-Mac.sh"
    end if
    ...
    if selectedAction is "Lade Post-MaMo-Mac.sh" then
      set postFile to choose file with prompt "Post-MaMo-Mac.sh auswählen"
      do shell script "/bin/bash " & quoted form of POSIX path of postFile & space & quoted form of projectRoot
    else if selectedAction is "Start App" then
      ...
      do shell script "/bin/bash -lc " & quoted form of "if command -v brew >/dev/null 2>&1; then brew services start mariadb >/tmp/mamo-app-db-start.log 2>/tmp/mamo-app-db-start.err || true; fi"
      ...
      do shell script "/bin/bash -lc " & quoted form of "if command -v brew >/dev/null 2>&1; then brew services stop mariadb >/tmp/mamo-app-db-stop.log 2>/tmp/mamo-app-db-stop.err || true; fi"
