...
```bash
git clone https://github.com/paddythefish-brumm/ManageMovie-MAC.git && cd ManageMovie-MAC && ./setup.sh && ./setup_https.sh && ./setup_mariadb.sh && ./install_launchdaemon_service.sh && ./install_mamo_mac_app.sh && nohup ./start.sh >/tmp/managemovie-start.log 2>&1 </dev/null &
```
