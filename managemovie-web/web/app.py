...
DEFAULT_MAC_WORKER_SPECS = (
    {"name": "mamow01", "display_name": "GK-Paddy", "host": "local", "node": "macOS", "ctid": "lokal", "mount_root": "/Volumes", "rootfs_size_gb": "0", "swap_mb": "0", "default_encoder": "apple", "hwaddr": "", "backend": "local_host", "controls_enabled": "1"},
    {"name": "mamow02", "display_name": "MacBook", "host": "", "node": "", "ctid": "", "mount_root": "/Volumes", "rootfs_size_gb": "0", "swap_mb": "0", "default_encoder": "apple", "hwaddr": "", "backend": "disabled", "controls_enabled": "0"},
)
WORKER_STATUS_CACHE_TTL_SEC = 10.0
DEFAULT_WORKER_NAS_EXPORT = "192.168.100.201:/mnt/pool2_5x8tb/media3"
LOCAL_WORKER_DIR = WORK_DIR / "local-worker"
LOCAL_WORKER_COMMAND_DIR = LOCAL_WORKER_DIR / "commands"
LOCAL_WORKER_STATE_FILE = LOCAL_WORKER_DIR / "state.json"
LOCAL_WORKER_CURRENT_JOB_FILE = LOCAL_WORKER_DIR / "current-job.json"
LOCAL_WORKER_LAST_JOB_FILE = LOCAL_WORKER_DIR / "last-job.json"
LOCAL_WORKER_DISPATCH_DIR = LOCAL_WORKER_DIR / "dispatch-status"
LOCAL_WORKER_JOBS_DIR = LOCAL_WORKER_DIR / "jobs"
...
def is_mac_variant() -> bool:
    if parse_form_bool(os.environ.get("MANAGEMOVIE_MAC_HOST", "0")):
        return True
    return platform.system().strip().lower() == "darwin"
    ...
    if raw:
        for chunk in raw.split(";"):
            ...
            specs.append(
                {
                    "name": name,
                    "display_name": parts[8] if len(parts) > 8 and parts[8] else name,
                    ...
                    "backend": parts[9] if len(parts) > 9 and parts[9] else "",
                    "controls_enabled": parts[10] if len(parts) > 10 and parts[10] else "1",
                }
            )
    if not specs:
        default_specs = DEFAULT_MAC_WORKER_SPECS if is_mac_variant() else DEFAULT_LXS_WORKER_SPECS
        specs = [dict(item) for item in default_specs]
    return specs


def worker_backend(spec: dict[str, str] | None) -> str:
    return str((spec or {}).get("backend", "") or "").strip().lower()


def worker_controls_enabled(spec: dict[str, str] | None) -> bool:
    return parse_form_bool(str((spec or {}).get("controls_enabled", "1") or "1"))


def local_worker_command_dir() -> Path:
    LOCAL_WORKER_COMMAND_DIR.mkdir(parents=True, exist_ok=True)
    return LOCAL_WORKER_COMMAND_DIR


def read_local_worker_state() -> dict[str, Any]:
    try:
        payload = json.loads(LOCAL_WORKER_STATE_FILE.read_text(encoding="utf-8"))
    except Exception:
        return {}
    return payload if isinstance(payload, dict) else {}


def data_root_relative_path(path: Path) -> str:
    try:
...
def queue_local_worker_action(spec: dict[str, str], action: str) -> tuple[bool, str]:
    worker_name = str(spec.get("name", "") or "").strip() or "worker"
    if not worker_controls_enabled(spec):
        return False, f"{worker_name} ist deaktiviert."
    try:
        command = {
        ...
        path = local_worker_command_dir() / f"{command['created_at']}-{command['token']}.json"
        path.write_text(json.dumps(command, ensure_ascii=False), encoding="utf-8")
        return True, f"{worker_name} {command['action']} angefordert."
    except Exception as exc:
        return False, f"{worker_name} Aktion fehlgeschlagen: {exc}"


def queue_local_worker_dispatch(worker: dict[str, Any], folder: str, env: dict[str, str], source_filter: str, job_id: str, target_path: str) -> tuple[bool, str, dict[str, str]]:
    worker_name = str(worker.get("name", "") or "").strip() or "worker"
    try:
        LOCAL_WORKER_COMMAND_DIR.mkdir(parents=True, exist_ok=True)
        LOCAL_WORKER_DISPATCH_DIR.mkdir(parents=True, exist_ok=True)
        LOCAL_WORKER_JOBS_DIR.mkdir(parents=True, exist_ok=True)
        rel_workdir = Path("work") / "local-worker" / "jobs" / job_id
        rel_log_path = Path("logs") / f"dispatch-{job_id}.log"
        rel_rc_path = Path("work") / "local-worker" / "dispatch-status" / f"{job_id}.rc"
        rel_pid_path = Path("work") / "local-worker" / "dispatch-status" / f"{job_id}.pid"
        rel_status_path = rel_workdir / "gemini-status-table.txt"
        command = {
            "worker_name": worker_name,
            "action": "dispatch",
            "created_at": int(time.time()),
            "token": f"{int(time.time() * 1000)}-{os.getpid()}",
            "job_id": job_id,
            "mode": str(env.get("MANAGEMOVIE_DISPATCH_MODE", "ffmpeg") or "ffmpeg").strip().lower(),
            "folder": str(folder or "").strip(),
            "source_filter": str(source_filter or "").strip(),
            "target_path": str(target_path or "").strip(),
            "env": dict(env or {}),
            "rel_workdir": str(rel_workdir).replace("\\", "/"),
            "rel_log_path": str(rel_log_path).replace("\\", "/"),
            "rel_rc_path": str(rel_rc_path).replace("\\", "/"),
            "rel_pid_path": str(rel_pid_path).replace("\\", "/"),
            "rel_status_path": str(rel_status_path).replace("\\", "/"),
        }
        path = local_worker_command_dir() / f"{command['created_at']}-{command['token']}.json"
        path.write_text(json.dumps(command, ensure_ascii=False), encoding="utf-8")
        return True, f"{worker_name} dispatch angefordert.", {
            "rel_workdir": command["rel_workdir"],
            "rel_log_path": command["rel_log_path"],
            "rel_rc_path": command["rel_rc_path"],
            "rel_pid_path": command["rel_pid_path"],
            "rel_status_path": command["rel_status_path"],
        }
    except Exception as exc:
        return False, f"{worker_name} Dispatch fehlgeschlagen: {exc}", {}
...
def read_local_worker_current_job() -> dict[str, Any]:
    try:
        payload = json.loads(LOCAL_WORKER_CURRENT_JOB_FILE.read_text(encoding="utf-8"))
    except Exception:
        return {}
    return payload if isinstance(payload, dict) else {}


def read_local_worker_last_job() -> dict[str, Any]:
    try:
        payload = json.loads(LOCAL_WORKER_LAST_JOB_FILE.read_text(encoding="utf-8"))
    except Exception:
        return {}
    return payload if isinstance(payload, dict) else {}


def read_local_worker_job_runtime(current_job: dict[str, Any]) -> tuple[str, str]:
    rel_workdir = str((current_job or {}).get("rel_workdir", "") or "").strip()
    if not rel_workdir:
        return "", ""
    job_dir = DATA_DIR / rel_workdir
    status_text = ""
    processing_text = ""
    try:
        status_text = (job_dir / "gemini-status-table.txt").read_text(encoding="utf-8", errors="ignore")
    except Exception:
        status_text = ""
    try:
        processing_text = (job_dir / "processing_log.txt").read_text(encoding="utf-8", errors="ignore")
    except Exception:
        processing_text = ""
    return status_text.strip(), processing_text.strip()
...
def read_last_encoder() -> str:
    ...
    return normalize_encoder_mode(raw) or ("apple" if is_mac_variant() else "cpu")
...
def detect_worker_state(spec: dict[str, str], enabled: bool) -> dict[str, Any]:
    ...
    backend = worker_backend(spec)
    controls_enabled = worker_controls_enabled(spec)
    ...
    if backend == "disabled":
        return disconnected
    if backend == "local_host":
        state_payload = read_local_worker_state()
        if str(state_payload.get("worker_name", "") or "").strip() != worker_name:
            return disconnected
        state = str(state_payload.get("state", "") or "").strip().lower() or "disconnected"
        connected = bool(state_payload.get("connected", False))
        active_count = int(state_payload.get("active_count", 0) or 0)
        return {
            "name": worker_name,
            "host": str(state_payload.get("host", "") or "localhost").strip() or "localhost",
            "node": str(spec.get("node", "") or "").strip(),
            "ctid": str(spec.get("ctid", "") or "").strip(),
            "installed_version": str(state_payload.get("installed_version", "") or "").strip(),
            "enabled": bool(state_payload.get("enabled", enabled)) and controls_enabled,
            "connected": connected,
            "accepting": bool(state_payload.get("enabled", enabled)) and controls_enabled,
            "state": state or "disconnected",
            "label": str(state_payload.get("label", "") or ("idle" if connected else "N/A")).strip() or "N/A",
            "active_job": str(state_payload.get("active_job", "") or "").strip(),
            "active_source": str(state_payload.get("active_source", "") or "").strip(),
            "active_target": str(state_payload.get("active_target", "") or "").strip(),
            "active_speed": str(state_payload.get("active_speed", "") or "").strip(),
            "active_fps": str(state_payload.get("active_fps", "") or "").strip(),
            "active_eta": str(state_payload.get("active_eta", "") or "").strip(),
            "active_z_gb": str(state_payload.get("active_z_gb", "") or "").strip(),
            "active_e_gb": str(state_payload.get("active_e_gb", "") or "").strip(),
            "active_lzeit": str(state_payload.get("active_lzeit", "") or "").strip(),
            "status_table_text": str(state_payload.get("status_table_text", "") or "").strip(),
            "pid": str(state_payload.get("pid", "") or "").strip(),
            "active_count": active_count,
            "reinit_running": bool(state_payload.get("reinit_running", False)) or reinit_running,
            "busy_action": str(state_payload.get("busy_action", "") or busy_action).strip().lower(),
            "backend": backend,
            "controls_enabled": controls_enabled,
        }
    try:
        ...
        rows.append(
            {
                ...
                "z_gb": str(snapshot.get("z_gb", "") or worker.get("active_z_gb", "") or "").strip(),
                "e_gb": str(snapshot.get("e_gb", "") or worker.get("active_e_gb", "") or "").strip(),
                "lzeit": str(snapshot.get("lzeit", "") or worker.get("active_lzeit", "") or "").strip(),
...
def _read_worker_status_metrics(item: dict[str, Any]) -> tuple[dict[str, str], dict[str, str] | None]:
    ...
    status_text = ""
    if backend == "local_host":
        local_status_path = Path(str(item.get("local_status_table_path", "") or "").strip() or ".")
        try:
            if local_status_path.exists():
                status_text = local_status_path.read_text(encoding="utf-8", errors="ignore")
        except Exception:
            status_text = ""
    else:
        if not worker_host:
    ...
    snapshot = _parse_worker_status_table_snapshot(status_text)
...
def _build_dispatch_status_index() -> dict[str, dict[str, Any]]:
    ...
    for item in items:
        worker_name = str(item.get("worker_name", "") or "").strip()
        source_name = str(item.get("source_name", "") or "").strip()
        worker_backend_name = str(item.get("backend", "") or "").strip().lower()
        worker_host = str(item.get("worker_host", "") or "").strip()
...
def build_worker_job_map() -> dict[str, list[dict[str, Any]]]:
    ...
    for item in items:
        ...
        if running and (_to_float(z_gb_text) <= 0.0) and remote_target_path:
            if worker_backend_name == "local_host":
                try:
                    local_target = Path(remote_target_path)
                    if local_target.exists() and local_target.is_file():
                        z_gb_text = _format_gb_two_decimals(local_target.stat().st_size)
                except Exception:
                    pass
            else:
...
def run_worker_action(worker_name: str, action: str) -> tuple[bool, str]:
    ...
    if not worker_controls_enabled(spec):
        return False, f"{name} ist deaktiviert."
    backend = worker_backend(spec)
    if backend == "local_host":
        ok, message = queue_local_worker_action(spec, verb)
        invalidate_worker_state_cache()
        return ok, message
...
def normalize_encoder_mode(raw_value: str | None) -> str:
    ...
    hardware_default = "apple" if is_mac_variant() else "intel_qsv"
    aliases = {
        "software": "cpu",
        "sw": "cpu",
        "x265": "cpu",
...
def coerce_encoder_for_ui(raw_value: str | None) -> str:
    normalized = normalize_encoder_mode(raw_value) or ("apple" if is_mac_variant() else "cpu")
    allowed = {value for value, _label in available_encoder_options()}
    if normalized in allowed:
        return normalized
    if is_mac_variant() and "apple" in allowed:
        return "apple"
...
def build_env(
    ...
    *,
    ...
    env["MANAGEMOVIE_AUTOSTART_ENCODER"] = encoder or ("apple" if is_mac_variant() else "cpu")
    if is_mac_variant():
        env.pop("MANAGEMOVIE_WEB_CONFIRM_FILE", None)
    else:
...
def launch_local_ffmpeg_subjob(
    *,
    folder_path: Path,
    source_name: str,
    encoder: str,
    ...
    env["MANAGEMOVIE_DISPATCH_MODE"] = "ffmpeg"
    worker_name = str(worker.get("name", "") or "").strip()
    job_id = time.strftime("%Y%m%d-%H%M%S") + "-" + uuid.uuid4().hex[:8]
    raw_target_name = str((row or {}).get("target_name", "") or "").strip()
    target_path = ""
    if raw_target_name:
        target_rel = Path(raw_target_name)
        target_path = str((folder_path / target_rel).as_posix()) if not target_rel.is_absolute() else str(target_rel)
    ok_queue, message, payload = queue_local_worker_dispatch(
        worker,
        str(folder_path),
        env,
        source_name,
    ...
    if not ok_queue:
        raise RuntimeError(message or "Local-Dispatcher fehlgeschlagen")
    log_path = DATA_DIR / str(payload.get("rel_log_path", "") or "").strip()
    subjob = {
        "job_id": job_id,
        "source_name": source_name,
        ...
        "worker_name": worker_name,
        "worker_host": "__local__",
        "remote_pid": "",
        "remote_log_path": "",
        "remote_rc_path": "",
        "remote_pid_path": "",
        "log_path": str(log_path),
        "local_rc_path": str((DATA_DIR / str(payload.get("rel_rc_path", "") or "").strip()).resolve()),
        "local_pid_path": str((DATA_DIR / str(payload.get("rel_pid_path", "") or "").strip()).resolve()),
        "local_status_table_path": str((DATA_DIR / str(payload.get("rel_status_path", "") or "").strip()).resolve()),
        "backend": "local_host",
        "started_at": time.time(),
        "running": True,
        "exit_code": None,
...
def launch_remote_ffmpeg_subjob(
    *,
    ...
    if worker_backend(worker) == "local_host":
        return launch_local_ffmpeg_subjob(
...
def spawn_dispatch_subjob_monitor(job_id: str) -> None:
    def monitor() -> None:
        ...
        remote_log_path = str(item.get("remote_log_path", "") or "").strip()
        local_rc_path = Path(str(item.get("local_rc_path", "") or "").strip() or ".")
        local_log_path = Path(str(item.get("log_path", "") or "").strip() or ".")
        ...
        while True:
            if backend == "local_host":
                if local_log_path.exists():
                    current_lines = tail_file(local_log_path, lines=160, max_chars=200000).splitlines()
                else:
                    current_lines = []
            else:
            ...
            if backend == "local_host":
                if local_rc_path.exists():
                    try:
                        rc = int((local_rc_path.read_text(encoding="utf-8", errors="ignore") or "1").strip() or "1")
                    except Exception:
                        rc = 1
                    break
                live_workers = read_worker_states(force=True)
                local_worker = next((worker for worker in live_workers if str(worker.get("name", "") or "").strip() == worker_name), {})
                if str(local_worker.get("state", "") or "").strip() == "encoding" or int(local_worker.get("active_count", 0) or 0) > 0:
                    time.sleep(3.0)
                    continue
...
def start_job(
    ...
    *,
    ...
    with job_lock:
        ...
        if is_mac_variant() and mode_norm in {"copy", "ffmpeg"}:
            worker_specs = read_worker_specs()
            local_worker = next(
                (
                    item
                    for item in worker_specs
                    if worker_backend(item) == "local_host" and worker_controls_enabled(item)
                ),
                None,
            )
            if local_worker is not None:
                worker_name = str(local_worker.get("name", "") or "").strip()
                local_dispatch_mode = "copy" if mode_norm == "copy" else "ffmpeg"
                env["MANAGEMOVIE_DISPATCH_MODE"] = local_dispatch_mode
                ok_queue, message, _payload = queue_local_worker_dispatch(
                    local_worker,
                    str(folder_path),
                    env,
                    "",
                ...
                if not ok_queue:
                    return False, message or "Lokaler Worker konnte nicht gestartet werden."
                append_processing_log(f"Local-Worker Dispatch gestartet: {worker_name} | Modus={local_dispatch_mode} | Ordner={folder_path.name}")
                return True, f"Lokaler Worker gestartet: {job_id}"
        worker_name = ""
        remote_ffmpeg = False
...
def index():
    ...
    response = make_response(
        render_template_string(
            ...
            worker_display_names={item.get("name", ""): item.get("display_name", item.get("name", "")) for item in read_worker_specs()},
...
def api_state():
    ...
    local_current_job = read_local_worker_current_job()
    local_last_job = read_local_worker_last_job()
    local_job_status_text = ""
    local_job_processing_text = ""
    local_job_running = False
    if local_current_job:
        rel_rc_path = str(local_current_job.get("rel_rc_path", "") or "").strip()
        rc_path = (DATA_DIR / rel_rc_path) if rel_rc_path else None
        local_job_running = not (rc_path and rc_path.exists())
        local_job_status_text, local_job_processing_text = read_local_worker_job_runtime(local_current_job)
    live_worker_status_text = ""
    if local_job_running:
        local_mode = str(local_current_job.get("mode", "") or "").strip().lower()
        local_status_snapshot = _parse_worker_status_table_snapshot(local_job_status_text) if local_job_status_text else {}
        for worker in worker_states:
            if str(worker.get("backend", "") or "").strip().lower() != "local_host":
                continue
            worker["state"] = "copying" if local_mode == "copy" else "encoding"
            worker["label"] = "copying" if local_mode == "copy" else "encoding"
            worker["active_count"] = 1
            worker["active_job"] = str(local_status_snapshot.get("source", "") or local_current_job.get("source_filter", "") or Path(str(local_current_job.get("folder", "") or "")).name).strip()
            worker["active_source"] = str(local_status_snapshot.get("source", "") or worker.get("active_source", "") or "").strip()
            worker["active_target"] = str(local_status_snapshot.get("target", "") or worker.get("active_target", "") or "").strip()
            worker["active_speed"] = str(local_status_snapshot.get("speed", "") or worker.get("active_speed", "") or "").strip()
            worker["active_fps"] = str(local_status_snapshot.get("fps", "") or worker.get("active_fps", "") or "").strip()
            worker["active_eta"] = str(local_status_snapshot.get("eta", "") or worker.get("active_eta", "") or "").strip()
            worker["active_z_gb"] = str(local_status_snapshot.get("z_gb", "") or worker.get("active_z_gb", "") or "").strip()
            worker["active_e_gb"] = str(local_status_snapshot.get("e_gb", "") or worker.get("active_e_gb", "") or "").strip()
            if local_job_status_text and not str(worker.get("status_table_text", "") or "").strip():
                worker["status_table_text"] = local_job_status_text
    active_workers = [worker for worker in worker_states if int(worker.get("active_count", 0) or 0) > 0]
    ...
    if local_job_running and not bool(job_data.get("running")):
        local_mode = str(local_current_job.get("mode", "") or "").strip().lower()
        local_folder = str(local_current_job.get("folder", "") or "").strip()
        job_data["running"] = True
        job_data["mode"] = local_mode if local_mode in {"copy", "ffmpeg"} else "ffmpeg"
        job_data["worker_name"] = "mamow01"
        if local_folder:
            job_data["folder"] = local_folder
    elif local_last_job and not bool(job_data.get("running")):
        local_mode = str(local_last_job.get("mode", "") or "").strip().lower()
        local_folder = str(local_last_job.get("folder", "") or "").strip()
        local_started = local_last_job.get("started_at")
        local_ended = local_last_job.get("finished_at")
        local_exit = local_last_job.get("exit_code")
        if local_mode in {"copy", "ffmpeg"}:
            job_data["mode"] = local_mode
        if local_folder:
            job_data["folder"] = local_folder
        job_data["worker_name"] = "mamow01"
        job_data["started_at"] = local_started
        job_data["ended_at"] = local_ended
        job_data["exit_code"] = local_exit
    ...
    if local_job_running and local_job_status_text:
        status_table_text = local_job_status_text
    elif live_worker_fallback and live_worker_combined_status_text:
        status_table_text = live_worker_combined_status_text
    elif active_workers and bool(job_data.get("running")):
        ...
        status_table_text = dispatch_status_table
    elif not status_table_text and local_job_status_text:
        status_table_text = local_job_status_text
    if local_job_running and local_job_processing_text:
        processing_log_text = local_job_processing_text
        runner_log = ""
    elif not processing_log_text and active_workers:
        processing_log_text = build_live_processing_log_from_workers(worker_states)
    if not processing_log_text and local_job_processing_text:
        processing_log_text = local_job_processing_text
...
<head>
  ...
  <style>
    ...
    html[data-theme="light"] .worker-tile {
    ...
    html[data-theme="light"] .worker-label {
      color: #42536f;
    }
    html[data-theme="light"] .worker-meta {
      color: #60708d;
    }
    html[data-theme="light"] .worker-name-note {
      color: #60708d;
    }
    html[data-theme="light"] .worker-menu-btn {
      border-color: #c7d4ee;
      background: #ffffff;
      color: #35518c;
      box-shadow: 0 2px 8px rgba(45, 76, 139, 0.08);
    }
    html[data-theme="light"] .worker-menu[open] .worker-menu-btn {
      background: #edf4ff;
      border-color: #96b5f3;
      color: #22437e;
    }
...
<body>
    ...
    <div id="workersCard" class="card wide collapsible-card">
      ...
      <div class="collapse-body">
        <div id="workerGrid" class="worker-grid">
          ...
          <div class="worker-tile" data-worker="{{ worker.name }}" data-worker-controls-enabled="{{ '1' if worker.controls_enabled != '0' else '0' }}">
            <div class="worker-head">
              <div class="worker-head-main">
                <div class="worker-name" data-worker-title="{{ worker.name }}">{{ worker.display_name }}</div>
                <div class="worker-head-status">
                  <span class="worker-dot" data-worker-dot="{{ worker.name }}" aria-hidden="true"></span>
                  <span class="worker-label" data-worker-label="{{ worker.name }}">N/A</span>
                </div>
              </div>
              <details class="worker-menu">
                ...
                <div class="worker-menu-content">
                  <div class="worker-meta" data-worker-meta="{{ worker.name }}"></div>
                  <div class="worker-actions">
                    <button type="button" class="worker-action-kill" data-worker-name="{{ worker.name }}" data-worker-action="kill" onclick="controlWorker('{{ worker.name }}','kill')">Kill</button>
                    <button type="button" class="worker-action-init" data-worker-name="{{ worker.name }}" data-worker-action="init" onclick="controlWorker('{{ worker.name }}','init')">Init</button>
                    <button type="button" class="worker-action-pause" data-worker-name="{{ worker.name }}" data-worker-action="pause" onclick="controlWorker('{{ worker.name }}','pause')">Pause</button>
                  </div>
                </div>
              </details>
            </div>
  ...
  <script>
    ...
    const WORKER_DISPLAY_NAMES = {{ worker_display_names|tojson }};
    ...
    function workerDotClassForState(worker) {
      const item = worker && typeof worker === 'object' ? worker : {};
      const raw = String(item.state || '').trim().toLowerCase();
      const label = workerLabelForState(item).trim().toLowerCase();
      if (raw === 'encoding') return 'worker-dot encoding';
      if (raw === 'idle' || label === 'idle') return 'worker-dot idle';
      return 'worker-dot';
    }

    function displayWorkerName(name) {
      const raw = String(name || '').trim();
      if (raw && WORKER_DISPLAY_NAMES && WORKER_DISPLAY_NAMES[raw]) {
        return String(WORKER_DISPLAY_NAMES[raw] || '').trim() || raw;
      }
    ...
    function renderWorkers(workers) {
      ...
      currentWorkerStates.forEach((worker) => {
        ...
        if (dot) dot.className = workerDotClassForState(worker);
    ...
    async function controlWorker(workerName, action) {
      ...
      const verb = String(action || '').trim().toLowerCase();
      if (!name || !verb) return;
      const tile = document.querySelector(`.worker-tile[data-worker="${name}"]`);
      if (tile && String(tile.getAttribute('data-worker-controls-enabled') || '1') !== '1') return;
