...
def _parse_worker_job_metrics_from_text(text: str) -> dict[str, str]:
    ...
    for raw_line in str(text or "").splitlines():
        ...
        if speed_match and speed_match.group(1):
            ...
            speed = raw_speed
        fps_match = re.search(r"(?:\bFPS\s*[:=]\s*|\bfps=)([0-9]+(?:[.,][0-9]+)?)", line, re.IGNORECASE)
...
def _read_worker_job_metrics(item: dict[str, Any]) -> dict[str, str]:
    ...
    if status_metrics.get("speed") or status_metrics.get("fps"):
        WORKER_METRIC_CACHE[cache_key] = dict(status_metrics)
    ...
    if running and worker_host and remote_log_path:
        ...
        if metrics.get("speed") or metrics.get("fps"):
    ...
    try:
        ...
        pass
    return dict(WORKER_METRIC_CACHE.get(cache_key, {"speed": "", "fps": ""}))
...
def _build_dispatch_status_index() -> dict[str, dict[str, Any]]:
    ...
    for item in items:
        ...
        if not worker_name or not source_name:
            continue
        metrics, snapshot = _read_worker_status_metrics(item)
        entry = {
            ...
            "eta": str((snapshot or {}).get("eta", "") or "").strip(),
            "z_gb": str((snapshot or {}).get("z_gb", "") or "").strip(),
            "started_at": float(item.get("started_at", 0.0) or 0.0),
        }
...
def build_worker_job_map() -> dict[str, list[dict[str, Any]]]:
    ...
    for item in items:
        ...
        if not worker_name or not source_name:
            continue
        metrics, snapshot = _read_worker_status_metrics(item)
        if not (metrics.get("speed") or metrics.get("fps")):
            metrics = _read_worker_job_metrics(item)
...
def build_dispatch_status_table_text(start_folder: str) -> str:
    ...
    active_row = running_rows[0] if running_rows else None
    active_index = annotated_rows.index(active_row) + 1 if active_row in annotated_rows else 0
...
<body>
  ...
  <script>
    ...
    function workerLabelForState(worker) {
      ...
      if (item.reinit_running) return 'init läuft';
      if (!item.connected) return 'N/A';
      if (item.state === 'encoding') return 'encoding';
    ...
    function buildSummaryDetailsText(data = null, statusMeta = '', statusTable = '', activeStatusKey = '') {
      ...
      const runningWorkerNames = collectRunningWorkerNames();
      const logSpeed = extractSummarySpeedFromProcessingLog(data, mode);
      const logFps = extractSummaryFpsFromProcessingLog(data, mode);
      const logEta = extractSummaryEtaFromProcessingLog(data, mode);
      const isoProgress = parseIsoProgressFromProcessingLog(data);
      ...
      const speedText = (mode === 'c' || mode === 'f')
        ? formatSummarySpeedText(mode === 'f' ? (workerTotals.speedText !== '-' ? workerTotals.speedText : (logSpeed || metrics.speed || '-')) : (logSpeed || metrics.speed || '-'), mode)
        : '-';
      const fpsText = mode === 'f' ? (workerTotals.fpsText !== '-' ? workerTotals.fpsText : (logFps || metrics.fps || '-')) : '-';
      const etaRaw = String(meta['ETA:'] || '').trim() || '-';
      const laufz = runtimeFromJob(data, meta['Laufz.:'] || '');
      const ersparnis = String(meta['Ersparnis:'] || '').trim() || '-';
      const lines = [];
      const ratio = isoAnalyzeActive
        ...
        : (progress.activeRatio || activeMeta.ratio || '-');
      if (mode === 'c' || mode === 'f') {
        lines.push(`Aktiv: ${workerTotals.activeCount > 0 ? workerTotals.activeCount : (postStepLabel || ratio || '0')}`);
        if (runningWorkerNames.length > 0) {
          lines.push(`Worker: ${runningWorkerNames.join(', ')}`);
        }
      }
      if (isoAnalyzeActive) {
        lines.push(`Speed: ${isoProgress.speed || '-'}`);
