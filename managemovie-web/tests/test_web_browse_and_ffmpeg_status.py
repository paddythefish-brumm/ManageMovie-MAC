...
class WebBrowseAndFfmpegStatusTests(unittest.TestCase):
    ...
    def test_home_page_renders_relevant_encode_settings(self) -> None:
        with WEB_APP.app.test_client() as client:
            response = client.get("/")
        self.assertEqual(response.status_code, 200)
        html = response.get_data(as_text=True)
        self.assertIn('id="precheckEgbSetting"', html)
        self.assertIn('id="speedFallbackCopySetting"', html)
        self.assertIn("Pre-Check E-GB", html)
        self.assertIn("Speed-Fallback auf Copy", html)
        self.assertNotIn('encodeOnce4kSetting', html)
        self.assertNotIn('oneEncodePerWorkerSetting', html)
        self.assertNotIn('parallelJobLimitSetting', html)
        self.assertNotIn("1x Encode bei 4k", html)
        self.assertNotIn("Nur 1 Encode pro Worker", html)
        self.assertNotIn("Parallele Jobs", html)
        self.assertNotIn('skipH265EncodeSetting', html)
        self.assertLess(html.find('precheckEgbSetting'), html.find('plexIpSetting'))
    ...
    def test_api_state_does_not_reuse_last_runner_log_when_processing_log_is_empty_and_job_is_idle(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertNotIn("stale runner log", payload["processing_log"])
